...
get_mysql_root_password() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira senha para o usuario Vespertineweb e Banco de Dados (Não utilizar caracteres especiais):${GRAY_LIGHT}"
  printf "\n\n"
...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do REPOSITÓRIO da aplicação que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador aplicação, Selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar aplicação\n"
  printf "   [1] Atualizar aplicação\n"
  printf "   [2] Deletar aplicação\n"
  printf "   [3] Bloquear aplicação\n"
  printf "   [4] Desbloquear aplicação\n"
  printf "   [5] Alter. dominio aplicação\n"
  printf "\n"
  read -p "> " option
